...
void QuadEstimatorEKF::UpdateFromIMU(V3F accel, V3F gyro)
{
  ...
  // Create initial attitude quaternion from estimated roll, pitch, and yaw
  // Integrate gyroscope measurements to update attitude
  // Extract updated roll, pitch, and yaw from the attitude quaternion

  // Normalize yaw to the range [-π, π]
  // Update state with normalized yaw
  ...
  float predictedYaw = attitude.Yaw();

  predictedYaw = std::fmod(predictedYaw + F_PI, 2.0f * F_PI);
  if (predictedYaw < 0.0f)
      predictedYaw += 2.0f * F_PI;
  predictedYaw -= F_PI;
...
// Predict position (x, y, z) using current velocity
// Convert acceleration from body frame to inertial frame and remove gravity
// Predict velocity (x_dot, y_dot, z_dot) using inertial acceleration
  ...
  const V3F accInertial = attitude.Rotate_BtoI(accel);

  predictedState(3) += accInertial.x * dt;
  predictedState(4) += accInertial.y * dt;
  predictedState(5) += accInertial.z * dt;
...
void QuadEstimatorEKF::Predict(float dt, V3F accel, V3F gyro)
{
  ...
  // Position update due to velocity
    // Velocity update due to yaw affecting acceleration (RbgPrime · accel)

  gPrime(0, 3) = dt;
  gPrime(1, 4) = dt;
  gPrime(2, 5) = dt;
  for (int i = 0; i < 3; ++i) {
      gPrime(i + 3, 6) = (RbgPrime(i) * accel).sum() * dt;
  }
...
void QuadEstimatorEKF::UpdateFromMag(float magYaw)
{
  ...
  // Set Jacobian for yaw observation (hPrime = [0 0 0 0 0 0 1])
  hPrime(0, 6) = 1.0f;
